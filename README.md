...
This model assumes the conflict remains a conventional, geographically contained engagement in which the UAE maintains its non-combatant posture, the global financial system continues to function, and the dirham peg holds. Events that would render the model structurally inapplicable include: nuclear escalation, UAE entry as a direct combatant, NATO Article 5 invocation, dirham peg collapse, or a conflict widening that triggers synchronized global recession beyond the crude demand-drag term modeled here. If any of these obtains, the model's 48-month recovery framework is not the right analytical instrument. The model is also not designed to function as a live tracker — its parameters reflect structural relationships calibrated in March 2026 and should be recalibrated if the conflict ends, if S&P revises the sovereign rating, or if Fujairah's operational status changes materially.
